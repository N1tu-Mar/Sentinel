  ...
  return (
    <li className="border-l-2 border-rule pl-3.5 text-sm">
      <span className="flex items-baseline gap-2">
        <span className="w-20 shrink-0 text-xs font-medium">{system}</span>
        <span>{tool}</span>
      </span>
...
function Verify({ passed, label, expected, observed }: { passed: boolean; label: string; expected: string; observed: string }) {
  return (
    <li className={`rounded-md border-l-4 px-3.5 py-2 text-sm ${passed ? "border-green bg-green-wash" : "border-red bg-red-wash"}`}>
      <p className={`font-medium ${passed ? "text-green" : "text-red"}`}>{label}</p>
      <p className="num mt-0.5 font-mono text-xs text-ink/80 [overflow-wrap:anywhere]">
        expected {expected}
        <br />
        observed {observed}
      </p>
...
export function LandingPreview() {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-paper text-left text-ink shadow-xl shadow-slate-200/60">
      <div className="flex items-center justify-between gap-3 border-b border-rule bg-sheet px-4 py-2 text-xs text-muted">
        <span className="font-medium">Scenario preview</span>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-rule bg-sheet px-4 py-4 sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-sm text-muted">{reasonText("product_not_received")} · Sandbox</p>
        </div>
      ...
      <div className="grid gap-8 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        <div>
          <p className="text-sm font-semibold">What the agent did</p>
          <ol className="mt-3 space-y-2">
            ...
            <li className="my-3 rounded-md border-2 border-ink bg-sheet p-4">
              <p className="flex flex-wrap items-baseline justify-between gap-2">
                <span className="text-lg font-semibold">Submit evidence</span>
        ...
        <div>
          <p className="text-sm font-semibold">Final verification</p>
